...
public class TicketService {
    ...
    public Ticket validateTicketUsage(Long ticketId, Long userId) {
            ...
            case LANDLORD -> {
                if (!ticket.getUnit().getProperty().getLandlord().getUserId().equals(userId)) {
                    throw new ResponseStatusException(HttpStatus.FORBIDDEN,
                            "Ticket belongs to a unit that does not belong to you.");
                }
            }
            case MAINTENANCE -> {
    ...
    // check permisions for ui implementation (same as view maintenance history
    // method, but for viewing tickets)
    ...
    public Ticket createTicket(Ticket ticket, Long unitId, Long fixtureId, Long userId) {
        ...
        unit = switch (user.getRole()) {
            ...
            default -> unitService.getUnitById(unitId); // should never happen since validateUserRole() will throw the
    ...
    public Ticket assignTicket(Long ticketId, Long staffId, Long userId) {
        ...
        if (!ticket.getUnit().getProperty().getStaff().contains(staff)) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND,
                    "Staff member is not part of property's staff list.");
        }
    ...
    public Ticket setTicketPriority(Long ticketId, TicketPriority priority, Long userId) {
        ...
        if (ticket.getStatus().equals(TicketStatus.CANCELLED) || ticket.getStatus().equals(TicketStatus.CLOSED)) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                    "Ticket priority cannot be modified once closed or cancelled.");
        }
        if (!ticket.getStatus().equals(TicketStatus.OPEN) && user.getRole().equals(UserRole.TENANT)) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                    "Ticket priority cannot be modified by tenant once processed.");
        }
    ...
    // maintenance statistics view ###FOR LANDLORD### ###FOR MAINTENANCE### ###FOR
    // TENANTS###
    ...
    // maintenance statistics: view by fixture (shared between properties if
    // different properties have same fixture, it is fine to see that this fixture
    // has issue x)
